...
const DEFAULT_VARIANT = path.join(
  DIR,
  "../results/generated_results_api_consensus_20260614_195059/tables/all_cases_ai_only_consensus_prefill.csv",
);
...
const INPUTS = {
  ...
  human: path.join(DIR, "../inputs/annotations/human-iso-ranges.csv"),
...
function main() {
  ...
  const outPath = path.join(DIR, "../recomputed", `ai_variant_score_${label.replace(/[^A-Za-z0-9_-]+/g, "_")}.json`);
