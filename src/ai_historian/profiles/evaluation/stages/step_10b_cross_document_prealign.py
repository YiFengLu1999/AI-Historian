...
MIN_EPISODE_CONFIDENCE = float(os.getenv("AIH_CROSSDOC_PREALIGN_MIN_EPISODE_CONF", "0.72"))
...
def split_non_anchor_blocks(
    ...
    for block in blocks:
        ...
        should_split = (
            ...
            and not has_explicit_temporal_anchor(tm)
        )
        if not should_split:
            result.append(block)
            continue
...
def call_episode_context_alignment(
    ...
    prompt = episode_alignment_prompt(source_doc, target_doc, source_items, target_items, target_context_items)
    messages = [
    ...
    for _attempt in range(3):
        try:
            ...
            content = response.choices[0].message.content or "{}"
            return normalize_episode_context_response(validate_json_text(EpisodeContextResponse, content))
        except Exception as exc:
            last_error = exc
            messages = [
    ...
    print(f"Episode context alignment failed after retries: {type(last_error).__name__ if last_error else 'Unknown'}: {str(last_error)[:240] if last_error else ''}")
    return EpisodeContextResponse()
...
def apply_episode_context_alignment(
    ...
    for source_doc, target_doc in [(left_doc, right_doc), (right_doc, left_doc)]:
        ...
        stats["calls"] += 1
        response = call_episode_context_alignment(source_doc, target_doc, source_items, target_items, target_context_items)
        for item in response.contexts:
            ...
            accepted = (
                ...
                and item.relation in {"episode_context", "same_sequence_phase"}
                and item.confidence >= MIN_EPISODE_CONFIDENCE
                and source_quote_ok
                and target_quote_ok
            )
            if not accepted:
                if before_item is None or (after_item is None and item.source_anchor_after_timeblock_id):
            ...
            evidence = {
                ...
                "supporting_source_quote": item.supporting_source_quote,
                "supporting_target_quote": item.supporting_target_quote,
                "quote_verified": True,
    ...
    for blocks in timeblocks.values():
        for block in blocks:
            if not isinstance(block, dict):
                continue
            had_crossdoc = False
            for key in ("crossdoc_boundary", "crossdoc_prealign", "crossdoc_event_links", "crossdoc_time_evidence", "crossdoc_interval_evidence", "crossdoc_context_evidence", "event_cluster_id"):
                if key in block:
                    block.pop(key, None)
                    had_crossdoc = True
            if had_crossdoc:
...
def main() -> None:
    ...
    reporter = StepReporter("Step10b", total=1)
    ...
    if not case_doc_pairs:
        ...
        save_json(RUN_ROOT / "timeblock" / "step10b_crossdoc_prealign_report.json", report)
