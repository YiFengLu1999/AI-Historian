...
def batch_process_step5_to_step6(
    ...
    if not json_files:
        print(f"⚠️ 在目录中没有找到 json 文件: {step5_dir.resolve()}")
        return
    ...
    print(f"开始处理，共发现 {total} 个文件。")
    print(f"输入目录: {step5_dir.resolve()}")
    print(f"输出目录: {step6_dir.resolve()}")
    print("-" * 60)
    ...
    for idx, input_path in enumerate(json_files, 1):
        try:
            ...
            success += 1
            print(f"[{idx}/{total}] ✅ 成功: {input_path.name} -> {output_path.name}")
        except Exception as e:
            failed += 1
            print(f"[{idx}/{total}] ❌ 失败: {input_path.name}")
            print(f"    原因: {e}")

    print("-" * 60)
    print(f"处理完成: 成功 {success} 个，失败 {failed} 个，总计 {total} 个。")
    print(f"step6 输出目录: {step6_dir.resolve()}")
