...
def build_file_plan(
    ...
    for txt_path in txt_files:
        ...
        if output_file_name in seen_output_names:
            raise ValueError(f"生成了重复输出文件名：{output_file_name}")
...
def main():
    ...
    print(f"候选书目: {candidate_book_titles if candidate_book_titles else ['<未识别>']}")
    print(f"命中文献条目数: {len(candidate_books)}")
    print(f"当前文件夹人物: {folder_people}")
    print("-" * 60)
    ...
    for file_plan in file_plans:
        try:
            ...
            print(f"处理成功: {result['source_file']}")
            print(f"  传主: {result['source_person']}")
            print(f"  传记标题: {result['source_title']}")
            print(f"  chapter_id: {result['chapter_id']}")
            print(f"  book_uuid: {result['book_uuid']}")
            print(f"  目录命中: {result['matched_catalog']}")
            ...
            print(f"处理失败: {file_plan['source_file']}")
            print(f"  错误原因: {e}")
            print("-" * 60)
