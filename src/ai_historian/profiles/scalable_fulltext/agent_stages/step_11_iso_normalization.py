...
from ai_historian.profiles.scalable_fulltext.agent_stages.regnal_year_converter import (
    load_default_converter,
)
...
# 1) Paths
# =========================
TIMEBLOCK_INPUT_STEP = int(os.getenv("AIH_ISO_INPUT_STEP", "10"))
TIMEBLOCK_OUTPUT_STEP = int(os.getenv("AIH_ISO_OUTPUT_STEP", "11"))
STEP_LABEL = os.getenv("AIH_ISO_STEP_LABEL", "Step11")
RUN_ROOT: Path
...
def worker_call(tm: str, gran: str, model: str = DEFAULT_MODEL) -> Tuple[str, str, str, str]:
    ...
    status in {"ok","lookup","regnal","empty","cached","invalid","fail","no_tm"}
    """
    ...
    if REGNAL_YEAR_CONVERTER is None:
        raise RuntimeError("Regnal-year converter is not initialized; call main() first")
    regnal_conversion = REGNAL_YEAR_CONVERTER.convert(tm)
    if regnal_conversion and validate_iso(regnal_conversion.iso):
        if ENABLE_CACHE:
            with _cache_lock:
                _cache[key] = regnal_conversion.iso
        return (regnal_conversion.iso, "regnal", "", "")
...
def run_batch(concurrency: int = CONCURRENCY):
    ...
    reporter = StepReporter(STEP_LABEL, total=len(timeblock_files))
    reporter.start(
...
def main() -> None:
    ...
    global EXPERIMENT_ISO_LOOKUP, EXPERIMENT_ISO_LOOKUP_SOURCE
    global REGNAL_YEAR_CONVERTER, OPENAI_CLIENT

    RUN_ROOT = resolve_run_root(sys.argv[1] if len(sys.argv) > 1 else None)
    TIMEBLOCK_INPUT_DIR = timeblock_step_dir(RUN_ROOT, TIMEBLOCK_INPUT_STEP)
    SEQUENCE_INPUT_DIR = sequence_step_dir(RUN_ROOT, 8)
    OUTPUT_DIR = timeblock_step_dir(RUN_ROOT, TIMEBLOCK_OUTPUT_STEP)
    ...
    REGNAL_YEAR_CONVERTER = load_default_converter(PROJECT_ROOT)
    ...
    if REGNAL_YEAR_CONVERTER.records:
        rprint(
            "[bold cyan]regnal year converter:[/bold cyan]",
            f"{REGNAL_YEAR_CONVERTER.source} entries={len(REGNAL_YEAR_CONVERTER.records)}",
        )
